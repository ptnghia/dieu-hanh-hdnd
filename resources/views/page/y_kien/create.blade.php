            ...
            <div class="card">
                <div class="card-body">
                    <div class="card-title d-flex align-items-center">
                        ...
                        <h5 class="mb-0 text-primary">Ý kiến cử tri</h5>
                    </div>
                    <hr>
                    <form class="row g-3" method="POST" action="{{ route('y-kien-cu-tri.store')}}" id="upload_vanban">
                        @csrf
                        <input type="hidden" name="key_file" value="{{session('key_upload')}}">
                        <div class="col-12 col-lg-8 col-md-8">
                            <label for="tieu_de" class="col-form-label">Tiêu đề ý kiến cử tri</label>
                            <input type="text" placeholder="Nhập tiêu đề ý kiến cử tri"  name="tieu_de" class="form-control @error('tieu_de') is-invalid @enderror" value="{{old('tieu_de')}}" id="tieu_de">
                            @error('tieu_de')
                        ...
                        <div class="col-12">
                            <label class="col-form-label">Nội dung ý kiến cử tri</label>
                            <textarea name="noi_dung_y_kien" id="mytextarea" rows="3" class="form-control @error('noi_dung_y_kien') is-invalid @enderror">{{old('noi_dung_y_kien')}}</textarea>
                            @error('noi_dung_y_kien')
                                <span class="invalid-feedback" role="alert">
                                    <i>{{ $message }}</i>
                                </span>
                            @enderror
            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        <div>
                            <h6 class="mb-0">Chủ đề ý kiến cử tri</h6>
                        </div>
                    </div>
...
<script>
    ...
    $('.btn-del').click(function(){
        var data_id = $(this).attr('data_id');
        swal({
            title: "Xác nhận xóa chủ đề ý kiến cử tri?",
            text: "",
            icon: "warning",
            buttons: true,
            dangerMode: true,
    ...
    CKEDITOR.replace( 'noi_dung_y_kien', {
        toolbar: [
