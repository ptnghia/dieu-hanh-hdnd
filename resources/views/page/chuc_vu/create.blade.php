            ...
            <div class="card border-top border-0 border-4 border-primary">
                <div class="card-body p-5">
                    <div class="card-title d-flex align-items-center">
                        ...
                        <h5 class="mb-0 text-primary">Thêm mới chức vụ</h5>
                    </div>
                    <hr>
                    <form class="g-3" method="POST" action="{{ route('chuc-vu.store')}}">
                        @csrf
                        <div class="mb-3 row">
                            <label for="inputTen" class="text-end col-sm-2 col-form-label">Tên chức vụ</label>
                            <div class="col-sm-10">
                                <input type="text"  name="ten" class="form-control @error('ten') is-invalid @enderror" value="{{old('ten')}}" id="inputTen">
                                @error('ten')
                        ...
                        <div class="mb-3 row">
                            ...
                            <div class="col-sm-10">
                                <input type="number" name="stt" value="{{old('stt')}}" class="form-control @error('stt') is-invalid @enderror" id="inputStt">
                                @error('stt')
                                    <span class="invalid-feedback" role="alert">
                                        <i>{{ $message }}</i>
                                    </span>
                        ...
                        <div class="text-end">
                            <a href="{{route('chuc-vu.index')}}" class="btn btn-default px-3 btn-load"><i class="bx bx-undo"></i> Quay lại</a>
                            <button type="submit" name="save_exit" class="btn btn-primary px-3 btn-load"><i class="bx bx-save"></i> Lưu và thoát</button>
                            <button type="submit" name="save_new" class="btn btn-success px-3 btn-load"><i class="bx bx-save"></i> Lưu và thêm mới</button>
                        </div>
                    </form>
                </div>
            </div>
